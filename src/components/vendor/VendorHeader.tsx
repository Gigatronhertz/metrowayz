...
const VendorHeader = ({ onMenuClick }: VendorHeaderProps) => {
  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-64 h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-8 z-10 shadow-sm">
      ...
      {/* Mobile - Show logo instead of search */}
      <div className="flex-1 md:hidden">
        <h1 className="text-lg font-display font-bold text-gray-900">MetroWayz</h1>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button className="relative p-2.5 rounded-xl hover:bg-gray-50 transition-all duration-200">
          ...
          {notifications > 0 && (
            <span className="absolute top-0.5 right-0.5 bg-primary-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-semibold shadow-md">
              {notifications}
            </span>
          )}
        </button>
        ...
        <div className="flex items-center gap-2 lg:gap-3 pl-2 lg:pl-4 border-l border-gray-100">
          ...
          <div className="hidden lg:block">
            <p className="text-sm font-semibold text-gray-900">{user?.name || 'Vendor'}</p>
